			...
			if (strtolower($sheet->getName()) == 'barang') {
				foreach ($sheet->getRowIterator() as $num_row => $row) {
					...
					foreach ($field_name as $num_col => $field) {
						...
						if ($field == 'kategori') {
							$kategori[$val] = [
								'nama_kategori' => $val, 'deskripsi' => $val, 'icon' => 'far fa-circle', 'aktif' => 'Y', 'new' => 'Y', 'urut' => 1
							];
						}

						if ($field == 'satuan') {
							$satuan[$val] = [
								'nama_satuan' => $val, 'satuan' => $val
							];
						}
					}

					$barang[] = [
						'kode_barang' => $data_value['kode'], 'nama_barang' => $data_value['nama'], 'nama_kategori' => $data_value['kategori'], 'nama_satuan' => $data_value['satuan'], 'stok' => $data_value['stok'], 'stok_minimum' => $data_value['stok_minimum'], 'deskripsi' => $data_value['deskripsi'], 'harga_pokok' => $data_value['harga_pokok'], 'harga_jual_ecer' => $data_value['harga_jual_ecer'], 'harga_jual_grosir' => $data_value['harga_jual_grosir'], 'berat' => $data_value['berat'], 'barcode' => $data_value['barcode'], 'tgl_input' => date('Y-m-d H:i:s'), 'id_user_input' => $id_user
					];
				}
				break;
			}
