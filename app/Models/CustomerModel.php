			...
			if (strtolower($sheet->getName()) == 'data') {
				...
				foreach ($sheet->getRowIterator() as $num_row => $row) {
					...
					if ($data_value) {
						...
						$data_customer[] = [
							'nama_customer' => $data_value['nama_customer'], 'alamat_customer' => $data_value['alamat_customer'], 'id_wilayah_kelurahan' => $id_wilayah_kelurahan, 'no_telp' => $data_value['no_telp'], 'email' => $data_value['email']
						];
					}
