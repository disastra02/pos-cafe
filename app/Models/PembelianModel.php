...
class PembelianModel extends \App\Models\BaseModel
{
	...
	public function saveData()
	{
		...
		$data_db['no_invoice'] = $_POST['no_invoice']; // ambil dari form
		$data_db['id_supplier'] = $_POST['id_supplier']; // ambil dari form
		$data_db['id_gudang'] = $_POST['id_gudang']; // ambil dari form

		list($d, $m, $y) = explode('-', $_POST['tgl_invoice']); // ambil dari form
		$data_db['tgl_invoice'] = $y . '-' . $m . '-' . $d; // ad to array data DB
		...
		if ($_POST['id']) {
			...
			$query = $this->db->table('pembelian')->insert($data_db); // insert  ke table pembelian
		...
		$this->db->table('pembelian_detail')->delete(['id_pembelian' => $id_pembelian]); // delete id di table pembelian_detail 
		$this->db->table('pembelian_detail')->insertBatch($data_db_barang); // insert list barang pembelian
		...
		if ($_POST['using_pembayaran']) {
			...
			foreach ($_POST['tgl_bayar'] as $key => $val) {
				...
				$this->db->table('pembelian_bayar')->insert($data_db); // insert ke data pembayaran 
			}
		}
