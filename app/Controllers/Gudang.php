	...
	public function getDataDT()
	{
		...
		foreach ($query['data'] as $key => &$val) {
			...
			$val['ignore_search_action'] = '<div class="form-inline btn-action-group">'
				. btn_label(
					[
						'icon' => 'fas fa-edit', 'attr' => ['class' => 'btn btn-success btn-edit btn-xs me-1', 'data-id' => $val['id_gudang']], 'label' => 'Edit'
					]
				)
				. btn_label(
					[
						'icon' => 'fas fa-times', 'attr' => [
							'class' => 'btn btn-danger btn-delete btn-xs', 'data-id' => $val['id_gudang'], 'data-delete-title' => 'Hapus nama gudang : <strong>' . $val['nama_gudang'] . '</strong>'
						], 'label' => 'Delete'
					]
				) .
