	...
	public function getDataDTPenjualan()
	{
		...
		foreach ($query['data'] as $key => &$val) {
			...
			if ($this->model->getJumlahBarang($val['id_penjualan'], 2) == $this->model->getJumlahBarang($val['id_penjualan'])) {
				...
				$status_transaksi = '<span class="badge text-bg-warning">'.$this->model->getJumlahBarang($val['id_penjualan'], 100).' Pesanan</span><br><span class="badge text-bg-success">'.$this->model->getJumlahBarang($val['id_penjualan'], 1).' Selesai</span>';
			}
