...
class Pembelian_retur extends \App\Controllers\BaseController
{
	...
	public function notaReturPdf()
	{
		...
		if (!empty($_GET['email'])) {
			...
			$email_data = array(
				'from_email' => $email_config->from, 'from_title' => 'Jagowebdev.com', 'to_email' => $pembelian_retur['supplier']['email'], 'to_name' => $pembelian_retur['supplier']['nama_supplier'], 'email_subject' => 'Nota Retur: ' . $pembelian_retur['data']['no_nota_retur'], 'email_content' => '<h2>Yth. ' . $pembelian_retur['supplier']['nama_supplier'] . '</h2><p>Berikut terlampir nota retur pembelian dari kami ' . $pembelian_retur['supplier']['nama_supplier'] . '.<br/><br/><p>Salam</p>', 'attachment' => ['path' => $filepath, 'name' => $filename]
			);
