	...
	public function getDataDT()
	{
		...
		foreach ($query['data'] as $key => &$val) {
			...
			$val['ignore_search_action'] = btn_action([
				'edit' => ['url' => $this->config->baseURL . $this->currentModule['nama_module'] . '/edit?id=' . $val['id_supplier']], 'delete' => [
					'url' => '', 'id' =>  $val['id_supplier'], 'delete-title' => 'Hapus data supplier: <strong>' . $val['nama_supplier'] . '</strong> ?'
				]
			]);
			$no++;
		}
